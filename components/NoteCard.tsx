      ...
      <div className="flex justify-between items-start">
        <div className="flex-1 mr-4">
          <div className="flex items-center gap-2 mb-1">
            <span className="bg-red-50 text-red-800 text-xs font-bold px-2 py-0.5 rounded-full">
              {note.variety}
            </span>
            <span className="text-xs text-gray-400">{note.vintage}</span>
          </div>
          <h2 className="font-bold text-xl text-gray-900 leading-tight mb-1">{note.name}</h2>
          <p className="text-sm text-gray-600">{note.region}</p>

          <div className="mt-3 flex flex-wrap gap-1">
            {note.aromas.slice(0, 5).map((aroma) => (
              <span key={aroma} className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded">
                {displayAroma(aroma)}
              </span>
            ))}
        ...
        <div className="flex flex-col items-end">
          ...
          <span className="text-2xl font-bold text-gray-800">{note.rating.toFixed(1)}</span>
        </div>
      </div>
    </div>
  );
